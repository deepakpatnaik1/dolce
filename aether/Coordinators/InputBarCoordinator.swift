...
class InputBarCoordinator: ObservableObject {
    ...
    private let glassmorphicRenderer = GlassmorphicEffectRenderer.shared
    ...
    // MARK: - Visual Effect Management
    
    /// Get the current glassmorphic style for the input bar
    func getGlassmorphicStyle() -> GlassmorphicEffectStyle {
        return glassmorphicRenderer.createStyleForComponent(.inputBar)
    }
    
    /// Report Metal shader warning if encountered
    func reportMetalWarning() {
        glassmorphicRenderer.reportMetalWarning()
    }
